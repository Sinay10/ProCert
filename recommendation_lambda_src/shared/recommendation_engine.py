...
class RecommendationEngine:
    ...
    def _get_content_metadata(self, content_id: str) -> Optional[ContentMetadata]:
        ...
        try:
            # Since we only have content_id, we need to scan to find the item
            # This is not ideal but necessary since content_id is not unique across certifications
            ...
            if response['Items']:
                ...
                # Convert DynamoDB item to ContentMetadata
                content_data = {
                ...
                return ContentMetadata.from_dict(content_data)
            
            return None
            
        except Exception as e:
            logger.error(f"Error retrieving content metadata for {content_id}: {str(e)}")
            return None
    ...
    def _get_content_by_difficulty(self, difficulty: DifficultyLevel, 
        ...
        try:
            if certification_type:
                # Query by certification type and filter by difficulty
            ...
            for item in response['Items']:
                try:
                    ...
                    from .models import ContentMetadata
                    content = ContentMetadata.from_dict(content_data)
    ...
    def _get_content_by_category(self, category: str, 
                               certification_type: Optional[CertificationType]) -> List[ContentMetadata]:
        """Get content by category using the correct GSI."""
        try:
            if certification_type:
                # Use the CertificationTypeIndex GSI which has certification_type as partition key and category as sort key
                response = self.content_metadata_table.query(
                    IndexName='CertificationTypeIndex',
                ...
                # If no certification type specified, scan with filter (less efficient but works)
                response = self.content_metadata_table.scan(
